...
// (cosigner partial-signs first, the user signs last and combines) against pinned
...
func TestComputePartialAndFinalSignature2Of2(t *testing.T) {
	...
	cosignerKey := hexDecode("b6f14c73ee5269f5a13a11f48ad54306293ee134e924f680fcd35f615881105b")
	...
	cosignerPublicKeyBytes := secp256k1.PrivKeyFromBytes(cosignerKey).PubKey().SerializeCompressed()
	...
	require.NoError(t, err)
	cosignerNonce, err := MuSig2GenerateNonce(Musig2v100, cosignerSessionID, cosignerPublicKeyBytes)
	require.NoError(t, err)

	cosignerPartialSignature, err := ComputePartialSignature2Of2(
		msg,
		cosignerKey,
		userPublicKeyBytes,
		userNonce.PubNonce[:],
...
func TestComputeFullSignature2Of2(t *testing.T) {
	...
	toSign := sha256.Sum256([]byte("cosigner-2-of-2"))
	tweak := KeySpendOnlyTweak()

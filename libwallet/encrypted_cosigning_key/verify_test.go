...
func CreateFirstSession(houstonService service.HoustonService, t *testing.T) service.CreateFirstSessionOkJson {
	provider := DefaultProvider()
	strClientVersion, err := strconv.Atoi(provider.ClientVersion)
	if err != nil {
		t.Fatal(err)
	}
	sessionJson := service.CreateFirstSessionJson{
		Client: service.ClientJson{
		...
		GcmToken:        nil,
		PrimaryCurrency: "USD",
		BasePublicKey: service.PublicKeyJson{
...
func TestCosigningKeyProofHappyPath_Integration(t *testing.T) {
	...
	err = houstonService.ChallengeKeySetupStart(service.ChallengeSetupJson{
		Type:                "RECOVERY_CODE",
	...
	err = houstonService.ChallengeKeySetupFinish(service.ChallengeSetupVerifyJson{
	...
	decryptedKey, err := DecryptCosigningKey(recoveryCode, encryptedKey, libwallet.Regtest())
	if err != nil {
		t.Fatal(err)
	}
	...
	if !bytes.Equal(extractChainCode(decryptedKey), key.ChainCode()) {
...
func extractChainCode(key *libwallet.HDPrivateKey) []byte {
	keyBytes := base58.Decode(key.String())

	// The chain code starts at position 13 and is 32 byte long. See
	//github.com/btcsuite/btcutil/blob/b2bf7f89d674a3702182b7e15f52807896051af3/hdkeychain/extendedkey.go#L514-L517

	return keyBytes[13 : 13+32]
}

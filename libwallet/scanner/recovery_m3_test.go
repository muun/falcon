...
//
//   - collaborative (3-of-3): user + cosigner + lightning server.
//   - non-collaborative (2-of-2 + relative timelock): user + cosigner + older(N).
//
// A recovering user never has the lightning server key, so recovery MUST go through the
// non-collaborative path: the descriptor carries the lightning server's PUBLIC key only, so
// bitcoin core can satisfy the input solely via the timelocked user+cosigner branch once the UTXO
// matures.
//
...
func TestM3KitToBtcCore_Integration(t *testing.T) {
	...
	userKey, cosignerKey := decryptEmergencyKitKeys(
		t, encodedUserKey, encodedCosignerKey, recoveryCode, nil,
	)

	// The lightning server is a third party. For recovery we only ever need its PUBLIC key.
	lightningServerKey := newLightningServerKey(t)
	...
	// The wallet lacks the lightning server key, so core spends the timelocked user+cosigner branch
	// of every UTXO.
...
// newLightningServerKey builds a deterministic lightning-server key. Only its public key is ever
// used (it goes into the descriptor as an xpub), since recovery never spends the collaborative
// branch.
func newLightningServerKey(t *testing.T) *libwallet.HDPrivateKey {
	seed := make([]byte, 32)
	for i := range seed {
		seed[i] = 0x07
	}
	lightningServerKey, err := libwallet.NewMasterHDPrivateKeyFromBytes(
		seed, make([]byte, 32), libwallet.Regtest(),
	)
	if err != nil {
		t.Fatalf("Failed to build lightning server key: %v", err)
	}
	return lightningServerKey
}
...
// (user+cosigner xprivs, lightning server xpub).
func loadM3Wallet(
	t *testing.T,
	userKey, cosignerKey, lightningServerKey *libwallet.HDPrivateKey,
	...
	for _, desc := range walletDescriptors {
		descriptor := fmt.Sprintf(
			...
			userKey.String(),
			cosignerKey.String(),
			lightningServerKey.PublicKey().String(),
			blocksForExpiration,
		)
...
func fundM3Addresses(
	...
	addresses := generateM3Addresses(
		t, userKey, cosignerKey, lightningServerKey, blocksForExpiration,
	)
...
func generateM3Addresses(
	...
	for _, path := range derivationPaths {
		...
		derivedCosignerKey, err := cosignerKey.PublicKey().DeriveTo(path)
		if err != nil {
			t.Fatalf("Failed to derive cosigner key at %s: %v", path, err)
		}
		derivedLightningServerKey, err := lightningServerKey.PublicKey().DeriveTo(path)
		if err != nil {
			t.Fatalf("Failed to derive lightning server key at %s: %v", path, err)
		}

		addrV7, err := libwallet.CreateAddressV7(
			derivedUserKey, derivedCosignerKey, derivedLightningServerKey, blocksForExpiration,
		)
		if err != nil {

...
func createFundedPSBT(
	...
	result, err := rpc.RawRequest(
		...
		[]json.RawMessage{
			mustMarshal(
				[]any{}, // inputs (auto-select)
			),
			mustMarshal(
				[]map[string]any{{destAddr: amountStr}}, // outputs
			),
			mustMarshal(0), // locktime
			mustMarshal(
				map[string]any{"subtractFeeFromOutputs": []int{0}}, // options
			),
		},
	)
	if err != nil {

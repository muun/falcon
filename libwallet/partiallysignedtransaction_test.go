...
func TestPartiallySignedTransaction_SignV1(t *testing.T) {
	...
	// We dont need to use the cosignerKey in V1
...
func TestPartiallySignedTransaction_SignV2(t *testing.T) {
	...
	const (
		...
		hexCosignerSig1 = "3045022100d07028674c49d8dabc536db47f1371c2f61fc578cb2c8797a570e3176f5e91c902206a83db8ad5b63e88c48d0ae4e67646fcf6e33d0177a88996c15b280494885e7b01"                                                                                                                                                                                                                                       //nolint:lll
		...
		hexCosignerSig2 = "304402201b0c35179a5fa8e6255115450979a77dbb97d89157e236783df0312a5d7bdb2c022064bae7ad0cdc72e4339421067cc65e0c3d03690a5c2d98c32a6ef67f883558a001"                                                                                                                                                                                                                                         //nolint:lll
		...
		hexCosignerSig3 = "30440220076b14b1c906089546cb40ce05dab38f0388ca65d0bc5183d3c3f7dcb98be52c022001eea4635d56726d990daa92ac26c52c9030c96dddcc92e5d623546580aaaef401"                                                                                                                                                                                                                                         //nolint:lll
		...
		hexCosignerSig4 = "30440220145dcce0bf6cceda98b3a9635bd7611d92085ff3ad27690bcf471a6b39620e6c02205ca0a0bd93550e86468e236b291457a3ff84a3b5dedeb10067cc9d3233b5dafa01"                                                                                                                                                                                                                                         //nolint:lll
		...
		encodedCosignerKey = "tpubDBYMnFoxYLdMBZThTk4uARTe4kGPeEYWdKcaEzaUxt1cesetnxtTqmAxVkzDRou51emWytommyLWcF91SdF5KecA6Ja8oHK1FF7d5U2hMxX" //nolint:lll
	...
	cosignerSig1, _ := hex.DecodeString(hexCosignerSig1)
	txOut2, _ := hex.DecodeString(hexTxOut2)
	cosignerSig2, _ := hex.DecodeString(hexCosignerSig2)
	txOut3, _ := hex.DecodeString(hexTxOut3)
	cosignerSig3, _ := hex.DecodeString(hexCosignerSig3)
	txOut4, _ := hex.DecodeString(hexTxOut4)
	cosignerSig4, _ := hex.DecodeString(hexCosignerSig4)
	...
	cosignerKey, _ := NewHDPublicKeyFromString(encodedCosignerKey, basePath, Regtest())
	userKey, _ := NewHDPrivateKeyFromString(encodedUserKey, basePath, Regtest())
	signedRawTx, err := partial.Sign(userKey, cosignerKey)
...
func TestPartiallySignedTransaction_SignV3(t *testing.T) {
	const (
		...
		hexCosignerSig1 = "3045022100d138caf8d3c19db84363b33e1ad002e1aee7907302ab5110edaf78d980c94e48022019e841da8759f63596fbcd81a3544219573288877206f8f651cae1023c397f0c01"                                                                                                                                     //nolint:lll
		...
		encodedCosignerKey = "tpubDABPYHYrYQHXY2pYFdcsFd41aE2uZmMQZpRRGiKfgz7G7nU7PoSwrzMKeHHnoMjmn9woC87coUanF2T911R8X5HpUtZRJRf56u4r51gTrqD" //nolint:lll
...
func TestPartiallySignedTransaction_SignV5(t *testing.T) {
	var (
		...
		encodedCosignerKey = "tpubDBS2rf9CeryjGstPrQVSzQhLGLqFVqEq78xtK26h9fsN7udiokAMuu6DbSzwhSzqCwszcfC2L2zMYoFm9uoiJpkEwyUCuNr3j1XswbHcgAB"                                                                              //nolint:lll
	...
	inputList := &InputList{inputs: []Input{
		&input{
			...
			cosignerPublicNonce: hexToBytes(
...
func TestPartiallySignedTransaction_SignV6(t *testing.T) {
	var (
		...
		encodedCosignerKey = "tpubDBmgp5wQ4SYkroyXQG3SxUXVZsdmJnL89exWksCAEq9xzujjCd6jpKbYQyyVXLiQk4gBq8AaUULZDbwxFF8DhcTEPzDFYY8g2dsJ1x3xPwN"                                                                              //nolint:lll
...
func TestPartiallySignedTransaction_SignV7(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			userKey, cosignerKey, lightningServerKey :=
				masterKey(t, 0x01), masterKey(t, 0x02), masterKey(t, 0x03)
			derivedUserKey, err := userKey.DeriveTo(keyPath)
			require.NoError(t, err)
			derivedCosignerKey, err := cosignerKey.DeriveTo(keyPath)
			require.NoError(t, err)
			derivedLightningServerKey, err := lightningServerKey.DeriveTo(keyPath)
			require.NoError(t, err)

			address, err := CreateAddressV7(
				derivedUserKey.PublicKey(),
				derivedCosignerKey.PublicKey(),
				derivedLightningServerKey.PublicKey(),
				timelock,
			)
			require.NoError(t, err)
			...
			coin := &coinV7{
				...
				LightningServerKey:  lightningServerKey.PublicKey(),
			}

			spendingTx := spendTimelockedOutput(prevTx, tt.sequence)

			if tt.collaborative {
				// The server signs for cosigner and the lightning server; both signatures must be present
				...
				require.NoError(t, err)
				lightningServerPubKey, err := derivedLightningServerKey.PublicKey().ECPubKey()
				require.NoError(t, err)
				...
				require.NoError(t, err)
				coin.CosignerSignature, err = coin.signature(
					0,
					spendingTx,
					derivedCosignerKey,
					witnessScript,
				...
				require.NoError(t, err)
				coin.LightningServerSignature, err = coin.signature(
					0,
					spendingTx,
					derivedLightningServerKey,
					witnessScript,
				)
				require.NoError(t, err)

				// The user adds the final signature locally.
				require.NoError(t, coin.SignInput(0, spendingTx, userKey, cosignerKey.PublicKey()))
			} else {
				// The user signs with user and cosigner key.
				require.NoError(t, coin.FullySignInput(0, spendingTx, userKey, cosignerKey))
			}
...
func TestPartiallySignedTransaction_SignV7CollaborativeCrossCheck(t *testing.T) {
	...
	coin.CosignerSignature, err = coin.signature(0, spendingTx, derivedCosignerKey, witnessScript)
	require.NoError(t, err)
	coin.LightningServerSignature, err = coin.signature(
		0, spendingTx, derivedLightningServerKey, witnessScript,
	)
	require.NoError(t, err)
	require.NoError(t, coin.SignInput(0, spendingTx, userKey, cosignerKey.PublicKey()))
...
func TestPartiallySignedTransaction_SignV9(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.collaborative {
				// The server signs for cosigner and the lightning server.
				signCollaborativeV9(
					t,
					coin,
					spendingTx,
				...
				// The user signs the non-collaborative leaf with both the user and cosigner keys.
				require.NoError(t, coin.FullySignInput(0, spendingTx, userKey, cosignerKey))
			}
...
// signCollaborativeV9 adds the cosigner and lightning server signatures, simulating the server
// response.
func signCollaborativeV9(
	t *testing.T,
	coin *coinV9,
	spendingTx *wire.MsgTx,
	derivedUserKey, derivedCosignerKey, derivedLightningServerKey *HDPrivateKey,
	timelock int64,
) {
	t.Helper()
	...
	require.NoError(t, err)
	cosignerPriv, err := derivedCosignerKey.key.ECPrivKey()
	require.NoError(t, err)
	lightningServerPriv, err := derivedLightningServerKey.key.ECPrivKey()
	require.NoError(t, err)
	cosignerPub, lightningServerPub := cosignerPriv.PubKey(), lightningServerPriv.PubKey()
	...
	cosignerSessionID := bytes.Repeat([]byte{0x20}, 32)
	lightningServerSessionID := bytes.Repeat([]byte{0x30}, 32)
	...
	require.NoError(t, err)
	cosignerNonce, err := musig.MuSig2GenerateNonce(
		musig.Musig2v100,
		cosignerSessionID,
		cosignerPub.SerializeCompressed(),
	)
	require.NoError(t, err)
	lightningServerNonce, err := musig.MuSig2GenerateNonce(
		musig.Musig2v100,
		lightningServerSessionID,
		lightningServerPub.SerializeCompressed(),
	)
	require.NoError(t, err)

	cosignerPartialSig, err := musig.ComputePartialSignature3Of3(
		sigHash,
		cosignerPriv.Serialize(),
		userPub.SerializeCompressed(),
		lightningServerPub.SerializeCompressed(),
		userNonce.PubNonce[:],
		lightningServerNonce.PubNonce[:],
		cosignerSessionID,
		tweak,
	)
	require.NoError(t, err)
	lightningServerPartialSig, err := musig.ComputePartialSignature3Of3(
		sigHash,
		lightningServerPriv.Serialize(),
		userPub.SerializeCompressed(),
		cosignerPub.SerializeCompressed(),
		userNonce.PubNonce[:],
		cosignerNonce.PubNonce[:],
		lightningServerSessionID,
		tweak,
	)
	require.NoError(t, err)
	...
	coin.CosignerPubNonce = cosignerNonce.PubNonce
	copy(coin.CosignerPartialSig[:], cosignerPartialSig)
	coin.LightningServerPubNonce = lightningServerNonce.PubNonce
	copy(coin.LightningServerPartialSig[:], lightningServerPartialSig)
}
...
func TestPartiallySignedTransaction_SignAll(t *testing.T) {
	var (
		...
		encodedCosignerKey = "tpubDBCFedMe1hS3ba6qSTivfi2f6MieNcgms1b6b9KK1xD6wtvG82dSvYQFgQcF2MBs4kyWEp7MB8tXgYzxiYpxDBnSU2F1sxrict9bNikM9kc"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        //nolint:lll
...
func TestPartiallySignedTransaction_SignSubmarineSwapV1(t *testing.T) {
	const (
		...
		txCosignerSigHex1 = "3044022032b35746170883b2f46c2f14019eb95e2e7e4d800248e6a8b372e504dc48674b02202ff47b29abf8f1be8719e757cbd218a4111c214b0c1aa4bdfc7debaf1b46880f01" //nolint:lll
		...
		encodedCosignerKey = "tpubDBZaivUL3Hv8r25JDupShPuWVkGcwM7NgbMBwkhQLfWu18iBbyQCbRdyg1wRMjoWdZN7Afg3F25zs4c8E6Q4VJrGqAw51DJeqacTFABV9u8" //nolint:lll
	...
	cosignerSig1, _ := hex.DecodeString(txCosignerSigHex1)
...
func TestPartiallySignedTransaction_SignSubmarineSwapV2(t *testing.T) {
	const (
		...
		encodedCosignerKey = "tpubD6NzVbkrYhZ4Yg872usw1wxNYrpCsUmiG4faYMaogSFwJFX9sz8MrR6GNKg4qUDjb3KUYcC9nrUL7tQYfK441qkFP9pwsw6fb8gTW7vJjXq" //nolint:lll
	...
	cosignerKey, _ := NewHDPublicKeyFromString(encodedCosignerKey, "m", Regtest())
	...
	inputs := []Input{
		&input{
			...
			submarineSwapV2: inputSubmarineSwapV2{
				...
				cosignerPublicKey:   cosignerKey.Raw(),
...
func TestPartiallySignedTransaction_SignIncomingSwap(t *testing.T) {
	const (
		...
		cosignerSigHex = "3045022100c4bef5d32c5ed3530cd258df645dfb0298744dee7820095aca1a188a3b2138c102201669e21db8ee4d2b090cbb18e3e52bce40fc5e07be73c1be4d26c9f13c02e69701" //nolint:lll
	)
	...
	cosignerKey, _ := NewHDPublicKeyFromString(
		encodedCosignerKey,
	...
	cosignerSig, _ := hex.DecodeString(cosignerSigHex)
...
func TestVerificationResultFailedChecks(t *testing.T) {
	...
	// These names reach our error reporting, where they are grouped and filtered on. Renaming one
	// silently is what this asserts against.
	result := &VerificationResult{
		destination:  errors.New("wrong destination"),
		outputShape:  errors.New("too many outputs"),
		changeAmount: errors.New("short change"),
	}

	if got := result.FailedChecks(); got != "destination,outputShape,changeAmount" {
		t.Errorf("FailedChecks() = %q", got)
	}

	if got := (&VerificationResult{}).FailedChecks(); got != "" {
		t.Errorf("FailedChecks() on a clean result = %q, want empty", got)
	}
}
...
func TestPartiallySignedTransaction_Verify(t *testing.T) {
	...
	const (
		...
		// 1-in 2-out: destination one satoshi above the dust floor, plus change.
		hexTxDustPlus = "" //nolint:lll

		// 1-in 3-out: destination, change, and an output paying neither.
		hexTx3Out = "" //nolint:lll

		// 1-in 2-out: two outputs paying the destination, each the full approved amount.
		hexTxDupDest = "" //nolint:lll

		// 1-in 2-out: destination sitting exactly at the dust floor (546) plus change. Models a
		// submarine swap whose funding output was padded up to dust.
		hexTxDust = "" //nolint:lll
	...
	cosignerPublicKey, _ := NewHDPublicKeyFromString(
		encodedCosignerKey,
		basePath,
		Regtest())
	...
	tests := []struct {
		...
		fields fields
		args   args
		// wantErr covers any failed check, enforced or incubating.
		wantErr bool
		// wantMustNotSign covers only the enforced ones, which are the ones that block signing.
		wantMustNotSign bool
		// validEitherWay opts out of the invariant that a TX verifying as an alternative must not
		// also verify as a non-alternative, for the cases where the two are indistinguishable.
		validEitherWay bool
	}{
		{
		...
		{
			name: "change shorter than the debt accounts for",
			fields: fields{
				tx:     hexTx1,
				inputs: []Input{&firstInput, &secondInput},
			},
			args: args{
				expectations: &SigningExpectations{
					...
					expectedDebtInSat: 299,
				},
				userPublicKey:     userPublicKey,
				cosignerPublicKey: cosignerPublicKey,
			},
			wantErr:         true,
			wantMustNotSign: false,
		},
		{
		...
		},
		{
			...
			// The undeclared change output pays an address we never derived, so this is enforced
			// now. It used to be caught only by the output count.
			wantErr:         true,
			wantMustNotSign: true,
		},
		{
		...
		{
			// A swap padded up to the dust floor can't have its destination reduced any further, so
			// the alternative TX pays exactly the approved amount. That must remain signable:
			// paying the destination what the user approved is never theft. Regression test for a
			// false positive that refused real sub-dust lightning payments.
			//
			// Paying exactly the approved amount is also what a non-alternative does, so at the
			// dust floor the two are indistinguishable and both must verify.
			name: "alternative at the dust floor pays exactly the approved amount",
			fields: fields{
				tx:     hexTxDust,
		...
		{
			name: "change dropped within the debt",
			fields: fields{
				tx:     hexTx4,
				inputs: []Input{&firstInput, &secondInput},
			},
		...
		{
			name: "change removed to burn debt",
			fields: fields{
				tx:     hexTx4,
				inputs: []Input{&firstInput, &secondInput},
			},
		...
		{
			name: "third output paying neither the destination nor our change",
			fields: fields{
				tx:     hexTx3Out,
				inputs: []Input{&firstInput},
			},
		...
		{
			name: "alternative just above the dust floor pays the approved amount",
			fields: fields{
				tx:     hexTxDustPlus,
				inputs: []Input{&firstInput},
			},
		...
		{
			name: "no change and a fee within the debt",
			fields: fields{
				tx:     hexTx4,
				inputs: []Input{&firstInput, &secondInput},
			},
		...
		{
			name: "no change and a fee the debt cannot account for",
			fields: fields{
				tx:     hexTx4,
				inputs: []Input{&firstInput, &secondInput},
			},
		...
		{
			name: "two outputs paying the destination instead of change",
			fields: fields{
				tx:     hexTxDupDest,
				inputs: []Input{&firstInput},
			},
		...
		{
			// Two outputs paying the destination pay it twice over, and only the last one would be
			// compared against the approved amount. Must be blocked: the wallet spends more than
			// the user approved. Enforced, because the output count check is not.
			name: "two outputs paying the destination",
			fields: fields{
				tx:     hexTxDupDest,
				inputs: []Input{&firstInput},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resultNonAlternative := p.Verify(
				&nonAlternativeExpectations,
				tt.args.userPublicKey,
				tt.args.cosignerPublicKey,
			)
			resultAlternative := p.Verify(
				alternativeExpectations,
				tt.args.userPublicKey,
				tt.args.cosignerPublicKey,
			)

			// Any failed check, enforced or not, is what Verify used to return an error for.
			failed := func(result *VerificationResult) bool {
				return result.MustNotSign() || result.IncubatingCheckFailed()
			}

			failedNonAlternative := failed(resultNonAlternative)
			failedAlternative := failed(resultAlternative)

			t.Logf("test %v non-alternative returned %q", tt.name, resultNonAlternative.Summary())
			t.Logf("test %v alternative returned %q", tt.name, resultAlternative.Summary())

			result := resultNonAlternative
			...
			if tt.args.expectations.alternative {
				...
				if failedAlternative != tt.wantErr {
					t.Errorf("Verify() failures = %q, wantErr %v",
						resultAlternative.Summary(), tt.wantErr)
				}

			} else {

				if failedNonAlternative != tt.wantErr {
					t.Errorf("Verify() failures = %q, wantErr %v",
						resultNonAlternative.Summary(), tt.wantErr)
				}
			}

			if tt.validEitherWay {

				if failedNonAlternative || failedAlternative {
					t.Errorf("Verify() should verify both ways. non-alternative = %q, "+
						"alternative = %q",
						resultNonAlternative.Summary(), resultAlternative.Summary())
				}

			} else if !failedNonAlternative && !failedAlternative {
				t.Errorf("Verify() alternative and non-alternative TXs must not both verify")
			}

			// Only some failures may block signing, which is the whole point of the split.
			if result.MustNotSign() != tt.wantMustNotSign {
				t.Errorf("Verify() MustNotSign = %v, want %v. failures = %q",
					result.MustNotSign(), tt.wantMustNotSign, result.Summary())
			}
